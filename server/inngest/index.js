...
//Inngest function to send renninders
const reminders = inngest.createFunction(
    ...
    async ({ step }) => {
        ...
        const result = await step.run("send-all-reminders", async () => {
            return await Promise.allSettled(
                ...
                <br/><p> Enjoy the show! <br/> QuickShow Team</p>
    </div > `
        ...
        const fail = result.length - sent

        return {sent, fail, message: `Sent ${sent} reminders, ${fail} failed`}
    }
)


//Inngest function to send notification to all users when a new show is added
    ...
    async ({ event }) => {
        ...
        for (const user of users) {
            ...
            <div style="font-family: Arial, sans-serif; padding: 20px;">
                <h2>Hi ${userName}</h2>
                <p>We've added a new show to our library:</p>
